...
> **Statut :** validation automatisée OK (2026-06-23) — test in-game manuel recommandé  
> Branche : `feature/link-corpus-v2`

## Prérequis

- [x] `assets/link-corpus-import/wonderland_companion_link_corpus_v2_clean_compact.zip` présent
- [x] `npm run validate:link-corpus` → exit 0 (7500 scénarios, 15 compagnons)
- [x] `npm run import:link-corpus-v2` exécuté → `src/data/linkCorpusV2.json` (7 500 scénarios)
- [x] `npm run build` OK (HEAD, sans régression)
- [x] `npm run lint` OK (12 warnings préexistants, 0 errors)

## Validation automatisée (session 2026-06-23)

| Check | Résultat |
|-------|----------|
| `pickConversation` V2-first via `engine.ts` | ✅ code review |
| Fallback legacy `companionScenarios.generated.ts` | ✅ si pack V2 vide |
| Affinité → pool `minAffinity`/`maxAffinity` | ✅ |
| Build bundle (~36 Mo chunk principal) | ✅ accepté test |
...
- Browser MCP indisponible session autonome — cases desktop/mobile non cochées in-game.
