...
- Max **15 espèces** visibles en scène (reste en badge `+N`)
- Chibis sans asset PNG → fallback emoji
- Pas de sprites / animations finales

## MVP 4.1 — Vie chibis + assignation (2026-06-26)

### Scène

- Chibis par **espèce** avec badge `×N` (comme le refuge)
- Zone **travail** : déplacement entre filons du biome (wanderers + anti-collision)
- Zones **repos** / **repas** : chibis regroupés, max 15 espèces
- Tous les Myrions du **biome actif** (tous spots), pas seulement le spot sélectionné

### Drawer Myrions

- Liste par **espèce** (chibi + `×N` + rareté + efficacité/s)
- Tri : efficacité ↑↓, rareté, nom
- **Assignation par lot** : nombre (1→50, page 30, tous) × critère (liste triée / top efficacité)
- Pagination **30** lignes max par page
- Résumé raretés sur le spot (`SR ×4`, etc.)
