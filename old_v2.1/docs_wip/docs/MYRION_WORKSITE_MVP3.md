...
- Production auto chantier : **`silent: true`** → pas de toast global (voir polish fin de dev ci-dessous)

Fichiers : `RewardToastProvider.tsx`, `rewardToastEntries.ts`, `useRewardToasts.ts`, `rewardToastContext.ts`.

## Polish fin de dev — feedback production passive (à traiter)

> **Rappel :** ne pas improviser en cours de MVP. Revoir en fin de chantier pour lisibilité et satisfaction joueur.

L’auto est volontairement silencieuse (pas de toast global). Le joueur voit le débit (`Auto X/s`) mais peu de **retour visuel immédiat** sur la scène — ex. bulles `+0,35 Pierre` au-dessus du spot.

**Piste fin de dev (à designer, pas le prototype bulles pills rejeté) :**

- Feedback **local au chantier** uniquement (pas les toasts village groupés)
- Rythme calé sur le tick (~1 s) ou fusion douce si plusieurs ressources
- Lisible mobile, non intrusif, cohérent avec placeholders puis assets finaux
- Tester avec débit réel (~0,35/s Pierre) et faible débit (1 Myrion)

**Critères :** le joueur comprend que l’auto produit sans spam ; la barre ressources village reste secondaire pendant la session chantier.
