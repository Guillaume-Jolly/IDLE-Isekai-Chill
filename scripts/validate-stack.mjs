#!/usr/bin/env node
/**
 * Smoke test infra projet — npm run validate:stack
...
console.log('[validate:stack] Infra IDLE Isekai Chill\n')
...
if (fileExists('public/data/link-corpus/manifest.json', 'link-corpus manifest (lazy)')) {
  const manifest = readJson('public/data/link-corpus/manifest.json')
  const source = readJson('src/data/linkCorpusV2.json')
  if (manifest && source && manifest.scenarioCount !== source.scenarios?.length) {
    fail('link-corpus manifest', 'scenarioCount désaligné — npm run corpus:split-batches')
  } else if (manifest && source) {
    ok('link-corpus manifest aligné sur linkCorpusV2.json')
  }
}
